...
module.exports = async (pluginConfig, context) => {
  ...
  if (successComment === false) {
    ...
    const issues = allowRemoteIssueComments
      ? [...prs.map((pr) => pr.body), ...commits.map((commit) => commit.message)].reduce((issues, message) => {
          return message
            ? issues.concat(
                parser(message).actions.close.map((action) => ({
                  slug: action.slug ? action.slug : `${owner}/${repo}`,
    ...
    console.log(`allowRemoteIssueComments = ${allowRemoteIssueComments}`);
    await Promise.all(
      uniqBy([...prs, ...issues], 'number').map(async (issue) => {
        ...
        try {
          ...
          let issueRepo = repo;
          if (allowRemoteIssueComments && issue.slug) {
            [issueOwner, issueRepo] = issue.slug.split('/');
          }

          const comment = {owner: issueOwner, repo: issueRepo, issue_number: issue.number, body};
          debug('create comment: %O', comment);
          const {
            data: {html_url: url},
          } = await github.issues.createComment(comment);
          console.log('Added comment to issue #%d: %s', issue.number, url);
