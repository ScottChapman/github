  ...
  if (successComment === false) {
    ...
    const issues = [...prs.map((pr) => pr.body), ...commits.map((commit) => commit.message)].reduce(
      (issues, message) => {
        return message
          ? issues.concat(
              parser(message)
                .actions.close.map((action) => ({slug: action.slug ? action.slug : `${owner}/${repo}`, number: Number.parseInt(action.issue, 10)}))
            )
          : issues;
      },
      []
    ...
    await Promise.all(
      uniqBy([...prs, ...issues], 'number').map(async (issue) => {
        ...
        try {
          let issue_owner = owner
          let issue_repo = repo;
          if (issue.slug) {
            [issue_owner, issue_repo] = issue.slug.split('/')
          }
          const comment = {owner: issue_owner, repo: issue_repo, issue_number: issue.number, body};
